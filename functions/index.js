    ...
    .onCreate((change, context) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            chatRef.onSnapshot(snapChat => {
                ...
                console.log('[CHAT DATA]', chatDoc, chatRef);
                let messageRef = chatRef.collection('messages').doc(messageId).onSnapshot(snapMessage => {
                    ...
                    let userTo = Object.keys(chatDoc.users).filter(key => { return(key !== toB64(userFrom))})[0];
                    console.log('[FROM]', userFrom);
                    console.log('[TO]', userTo);
                    db.collection('users').doc(fromB64(userTo)).collection('contacts').doc(toB64(userFrom)).set({
